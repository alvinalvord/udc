...
public class CreateBox extends VBox implements View {
	...
	private void initCreateBox() {
		...
		createFrom.getSelectionModel ().selectFirst ();		
		createTo.getSelectionModel ().selectFirst ();		
	}
		...
		save.setOnAction(e -> {
			if (!recurring.isSelected()) {
				if (createName.getText().isEmpty() || createFrom.getSelectionModel().isEmpty() || createTo.getSelectionModel().isEmpty())
					...
					try {
						if (time.parse(createFrom.getValue()).compareTo(time.parse(createTo.getValue())) >= 0)
							invalidAlert ("The start time " + createFrom.getValue() + " is after the end time " + createTo.getValue());
						else {
							String date = "" + calendar.getCurrentYear() + "-" + 
													(calendar.getCurrentMonth().getMonth() + 1) + "-" +
													calendar.getSelectedDate();
							String start_time = createFrom.getValue() + ":00";
							String end_time = createTo.getValue() + ":00";
							
							int retVal = dc.createInsert(createName.getText(), date, start_time, end_time,
										Integer.toString(DatabaseModel.getInstance().getUserID()));
										
							if (retVal == -1) {
								System.out.println("time conflict");
								invalidAlert ("time conflict on date: " + date + " from: " + start_time + " to: " + end_time);
							}
							else
								discard.fire ();
						...
						if (time.parse(createFrom.getValue()).compareTo(time.parse(createTo.getValue())) >= 0)
							...
							while (sDate.compareTo(eDate) <= 0) {
								...
								if (cal.get(Calendar.DAY_OF_WEEK) == i && days[i - 1].isSelected()) {
									retVal = dc.createInsert(createName.getText(), date, start_time, end_time,
											Integer.toString(DatabaseModel.getInstance().getUserID()));
									if (retVal == -1) {
										System.out.println("time conflict");
										invalidAlert ("time conflict on date: " + date + " from: " + start_time + " to: " + end_time);
