...
public class DatabaseModel extends Model {
	...
	public synchronized void checkUpdates () {
		try {
			...
			int n = stmt.executeUpdate ("update `appointments` set	`status_done` = '1' where `id` in (select `id` from (select * from `appointments`) as x where `date` <= date(now())`end_time` < time(now()) and `status_done` = '0');");
