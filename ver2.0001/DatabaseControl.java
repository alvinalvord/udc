...
public abstract class DatabaseControl {
	...
	public void setAgendaLabels (ArrayList<Label> labels, ArrayList <Integer> appointers) {
		for (int i = 0; i < labels.size (); i++) {
			if (appointers.get (i) == 0) 
				labels.get (i).setId ("agendaFree");
			else
				labels.get (i).setId ("agendaTaken");
	...
	public void setCalendarLabels (Label[] labels, int[] appointers) {
		for (int i = 0; i < labels.length; i++) {
			labels[i].getStyleClass ().add ("TimeLabelsNone");
			if (appointers[i] == 0)
				labels[i].getStyleClass ().add ("calendarFree");
			else if (appointers[i] > 0)
				labels[i].getStyleClass ().add ("calendarTaken");
		}
	}
