	...
	private void initHandlers() {
		...
		save.setOnAction(e -> {
			if (!recurring.isSelected()) {
				...
				// int n = dc.createInsert(createName.getText (),
				// calendar.getCurrentYear() + "-" +
				// (calendar.getCurrentMonth().getMonth() + 1) + "-" +
				// calendar.getSelectedDate(), createFrom.getValue(),
				// createTo.getValue(), Integer.toString(dc.getUserID()));
				/* try {
					SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
					Date sDate = sdf.parse(calendar.getCurrentYear() + "-" + (calendar.getCurrentMonth().getMonth() + 1)
							+ "-" + calendar.getSelectedDate());
					Date eDate = sdf.parse(recurringToDate.getText());
