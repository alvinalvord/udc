...
public class CreateBox extends VBox implements View{
	...
	private void initHandlers() {
		...
		save.setOnAction(e -> {
			if (!recurring.isSelected()) {
				int retVal = dc.createInsert (createName.getText (),"" + calendar.getCurrentYear () + "-" + (calendar.getCurrentMonth ().getMonth () + 1) + "-" + calendar.getSelectedDate (), createFrom.getValue () + ":00", createTo.getValue () + ":00");
				if (retVal == -1)
