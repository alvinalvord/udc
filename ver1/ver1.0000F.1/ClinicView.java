...
public abstract class ClinicView extends BorderPane implements View {
	...
	protected DatabaseControl dc;
	protected GregorianCalendarModel gcm;
	protected DatabaseModel dm;
	...
	public ClinicView (DatabaseControl dc) {
		super ();
	...
	protected void initRightPane () {
		...
		calendarDayScroller = new CalendarDayScroller (dc, gcm, dm);
		calendarWeekScroller = new CalendarWeekScroller (dc, gcm, dm);
		agendaDayScroller = new AgendaDayScroller (dc, gcm, dm);
		agendaWeekScroller = new AgendaWeekScroller (dc, gcm, dm);
		createBox = new CreateBox (dc, gcm);
	...
	protected void initHandlers () {
		...
		viewSelect.setOnAction (e -> {
			try {
				java.sql.ResultSet rs = dm.getData ("users", "`name` = '" + viewSelect.getValue () + "'");
				rs.next ();
	...
	protected abstract void setViewableSchedules ();
	
	protected abstract void setViewLabel ();
